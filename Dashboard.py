...
df2 = pd.read_csv('https://raw.githubusercontent.com/daniyar135/icebreaker/master/Silence.csv')
df3 = pd.read_csv('https://raw.githubusercontent.com/daniyar135/icebreaker/master/SV.CSV')
df4 = pd.read_csv('https://raw.githubusercontent.com/daniyar135/icebreaker/master/Top10.csv')

app = Dash(__name__)
server = app.server
figa = px.scatter(df1, y="Call duration", x="Number of silences", color="Recording ID", size="Silence Max")
figs = px.bar(df2, y="Silence duration", x="Contact ID")
figb = px.bar(df2, y="Contact ID", x="Class")
figsv = px.histogram(df3, y="Contact ID", x="Call duration")
figtop = px.histogram(df4, y="Contact ID", x="Call duration")
...
app.layout = html.Div(children=[
    ...
    dcc.Slider(
        ...
        step=None,
        value=df1['week'].min(),
        marks={str(week): ("week" + str(week)) for week in df1['week'].unique()} + df1['week'],
        id='week-slider'
    )
...
def update_figure(selected_week):
    ...
    fig1 = px.bar(filtered_df2, y="Silence duration", x="Recording ID")
    fig1.update_layout(transition_duration=500)

    fig2 = px.bar(filtered_df2, y="Recording ID", x="Class")
